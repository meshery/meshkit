...
var minKubernetesVersion = [3]int{1, 12, 0}
var minKubectlVersion = minKubernetesVersion

//GetK8sVersionInfo returns version.Info for the Kubernetes cluster.
func GetK8sVersionInfo(client *Client) (*version.Info, error) {
...
func CheckK8sVersion(versionInfo *version.Info) error {
	...
	if !isCompatibleVersion(minKubernetesVersion, apiVersion) {
		return fmt.Errorf("kubernetes is on version [%d.%d.%d], but version [%d.%d.%d] or more recent is required",
			...
			minKubernetesVersion[0], minKubernetesVersion[1], minKubernetesVersion[2])
	}
