...
func TestAssignDefaultsForCompDefs_StatusHandling(t *testing.T) {
	// Case 1: model status = ""
	...
	if comp.Status != nil {
		t.Errorf("expected component status to be nil when model status is empty")
	}

	// Case 2: model status = ignored
	ignored := model.ModelDefinitionStatus(entity.Ignored)
	modelDef.Status = ignored

	AssignDefaultsForCompDefs(comp, modelDef)

	if comp.Status == nil {
		t.Errorf("expected component status to be set when model status is ignored")
	}
}
